...
app.post('/newUser', async (req: Request, res:Response) => {
	...
	const type = req.body.type;
	try {
		const userInfo = await db.createUser(id,user,firstName,lastName,email,password,type);
...
app.post('/newEvent', async (req: Request, res: Response) => {
	...
	try {
		const eventInfo = await db.createEvent(id,title, location,  start,end, eoId);
		res.status(200).json({eventInfo: eventInfo, message: `Event ${title} Created`});
	} catch (err) {
		if (err instanceof Error) {
			res.status(400).json({error: "Bad Request", message: err.message});
		} else {
...
app.post('/event_student', async (req: Request, res: Response) => {
	...
	try {
		const eventStudentInfo = await db.createEventStudent(eventId,studentId);
		res.status(200).json({eventStudentInfo: eventStudentInfo, message: `Event ${eventId} for Student ${studentId} Created`});
	} catch (err) {
		if (err instanceof Error) {
			res.status(400).json({error: "Bad Request", message: err.message});
		} else {
